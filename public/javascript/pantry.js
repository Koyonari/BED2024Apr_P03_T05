...
document.addEventListener("DOMContentLoaded", (function() {
    ...
    products.forEach((value) => {
      ...
      newDiv.innerHTML = `
        ...
        <div class="add_remove_holder">
          <i id="ingredient_minus_icon" class='bx bxs-minus-circle' onclick="removeFromCard('${value.ingredient_id}')"></i>
          <input type="number" min="0" id="quantity-${value.ingredient_id}" placeholder="Enter amount">
          <i id="ingredient_plus_icon" class='bx bxs-plus-circle' onclick="addToCard('${value.ingredient_id}')"></i>
        </div>
  ...
  async function removeFromCard(ingredientId) {
    ...
    try {
      const response = await fetch(`http://localhost:3500/pantry/${pantryId}/deductIngredientQuantity`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
  ...
  async function addIngredient() {
    const ingredient_name = document.getElementById("popup_IngredientName").value;
  ...
  async function getFilteredRecipes(recipe_popup_id) {
    ...
    try {
      const response = await fetch(`http://localhost:3500/recipes/getfilteredrecipes`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${accessToken}`,
